"""Adapter-driven local builder core — shared worktree runner for codex and composer routes."""
...
class LocalBuilderResult:
    ...
    run_dir: str = ""
    agent_subprocess_invoked: bool = False
...
@dataclass
class LocalBuilderRunConfig:
    adapter_id: str
    execution_route: str
    worktree_owner: str
    agent_slug: str
    agent_output_relpath: str
...
def run_adapter_local_builder(
    repo_root: Path,
    *,
    task_path: Path,
    adapter: dict[str, Any],
    config: LocalBuilderRunConfig,
    evaluate_gates: Callable[..., Any],
    build_command_plan: Callable[..., CommandPlan],
    prepare_environment: Callable[[dict[str, Any]], tuple[dict[str, str], list[str], str | None]],
    resolve_executable: Callable[[str | None], str],
    base_sha: str | None = None,
    agent_executable: str | None = None,
    subprocess_runner: SubprocessRunner | None = None,
    skip_agent_subprocess: bool = False,
    fake_agent_exit: int = 0,
) -> LocalBuilderResult:
    """Execute one adapter-driven local-builder run."""
    ...
    handoff_rel = f"handoffs/{task_id}__{config.agent_slug}__to__claude.md"
    instructions_path = (
        repo_root
        / "runtime"
        / "dispatch"
    ...
    if skip_agent_subprocess:
        exit_code = fake_agent_exit
        stdout_path.write_text("agent subprocess skipped (test mode)\n", encoding="utf-8")
        stderr_path.write_text("", encoding="utf-8")
    else:
        agent_invoked = True
        argv = list(plan.argv)
        argv[0] = resolve_executable(agent_executable or argv[0])
    ...
    from dispatch.codex_local_builder_gate import evaluate_changed_paths_scope
    ...
    result_payload = {
        ...
        "agent_subprocess_invoked": agent_invoked,
    ...
    if config.adapter_id == "codex-restricted":
        result_payload["codex_subprocess_invoked"] = agent_invoked
