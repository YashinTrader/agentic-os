...
Only non-Claude `assignment_completed` events are eligible. The consumer fingerprints the stable event identity, creates an exclusive claim, and records `pending -> claimed -> review_starting -> review_running -> resolved`. `review_running` requires a real PID plus either a Claude session id or a schema-valid structured verdict. Claude runs with `--output-format stream-json`, and the dispatcher observes incremental stdout growth without conflating the 90-second activity window with the full review timeout. Process exit is parsed before launch classification, so buffered or late schema-valid verdicts are always recorded. A missing PID or genuinely absent activity is classified `failed_launch`; the launch is retried once. Failed reviews retain the original poke.
