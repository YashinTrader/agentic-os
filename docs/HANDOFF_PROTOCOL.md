...
5. `runtime/unittest_last_run.txt` must be generated by `scripts/run_tests.py` at `implementation_sha`. Its `commit_full` must equal `tests_commit_sha` (never hand-edited).
6. Commits after `tests_commit_sha` may touch only the **explicit** post-test allowlist (Phase 3.4.1: `verification-only-allowlist-v3`). Broad directory prefixes such as `docs/**`, `handoffs/**`, `tasks/**`, or `decisions/**` are **not** permitted.
7. Any change under `decisions/`, `dispatch/`, `scripts/`, `tests/`, `schemas/`, `protocol/`, `agents/`, `daemon/`, `dashboard/`, `orchestrator/`, or `integrations/` after `tests_commit_sha` invalidates verification and requires a new full test run.
