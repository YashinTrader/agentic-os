...
def build_claude_review_plan(
    *,
    ...
    # Deliver the review prompt on stdin. Windows resolves `claude` to claude.CMD
    # (cmd.exe batch wrapper); multiline text as a single argv after -p is mangled
    # and Claude receives an empty/broken prompt, producing conversational prose
    # instead of schema JSON. `claude -p` reads the prompt from stdin when omitted.
    argv = [
        executable,
        "-p",
        "--output-format",
    ...
    # Guard: never put multiline prompt text on argv (cmd.exe hazard).
    if any("\n" in part or "\r" in part for part in argv):
        return LaunchPlan(
            ...
            cwd=request.cwd or ".",
            executable=executable,
            blocked_reasons=["refusing multiline text in claude argv (use stdin)"],
        )
    ...
    redacted = list(redact_argv(argv))
    redacted.append("<prompt:stdin>")
    return LaunchPlan(
        agent="claude-reviewer",
        adapter="claude-reviewer",
        argv=argv,
...
_AUTH_FAILURE_RE = re.compile(
    r"401|oauth|not logged in|unauthori[sz]ed|token has been revoked|authentication",
    re.I,
)
_QUOTA_FAILURE_RE = re.compile(r"quota|rate[\s_-]?limit|usage[\s_-]?limit|\b429\b", re.I)


def _envelope_indicates_cli_error(stdout: str) -> bool:
    """True when Claude --output-format json envelope reports a transport/CLI error.

    Review *verdict text* often mentions OAuth/401 from handoffs under review; that must
    not be classified as reviewer authentication failure.
    """
    ...
    if outer.get("is_error") is True:
        return True
    if outer.get("api_error_status") not in (None, 0, "0"):
        return True
    subtype = str(outer.get("subtype") or "").lower()
    if subtype in {"error", "failure", "auth_error"}:
        return True
    return False
...
def classify_reviewer_failure(
    *,
    ...
    # Prefer stderr + envelope-level error flags. Do not scan free-text result/summary
    # bodies: a schema-valid review discussing historical OAuth blockers is success.
    stderr_text = stderr or ""
    envelope_error = _envelope_indicates_cli_error(stdout)
    probe_blob = stderr_text
    if envelope_error or exit_code not in (0, None):
        # Include stdout only when the process already looks failed.
        probe_blob = f"{stdout}\n{stderr_text}"

    if _AUTH_FAILURE_RE.search(probe_blob) and (envelope_error or exit_code not in (0, None) or _AUTH_FAILURE_RE.search(stderr_text)):
        return STATE_BLOCKED_AUTH, "authentication failure in Claude CLI output"
    if _QUOTA_FAILURE_RE.search(probe_blob) and (envelope_error or exit_code not in (0, None) or _QUOTA_FAILURE_RE.search(stderr_text)):
